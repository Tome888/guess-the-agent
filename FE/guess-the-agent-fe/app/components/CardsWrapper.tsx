...
export default function CardsWrapper({ token, userId, agentsArr, setAgents }: CardsWrapperProps) {
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6">
        {agentsArr.map((agent) => (
          <AgentCard agentArr={agentsArr} setAgents={setAgents} key={agent.id} {...agent} />
        ))}
      </div>
        </>
