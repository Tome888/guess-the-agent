...
export default function Chat({ turn, setTurn, token, userId }: ChatProps) {
  ...
  // Sanitize room id once
  const safeRoom = sanitizeString(roomData || "");

  useEffect(() => {
    if (!token || !userId || !safeRoom) return;

    const handleChat = (data: ChatType) => {
      // Sanitize received chat data
      const safeMsg = sanitizeString(data.msg || "");
      const safeAnswer = data.answer ? sanitizeString(data.answer) : null;

      const sanitizedData = { ...data, msg: safeMsg, answer: safeAnswer };

      setChatArr((prev) => {
        const existingIndex = prev.findIndex((msg) => msg.id === sanitizedData.id);
        if (existingIndex !== -1) {
          const updatedArr = [...prev];
          updatedArr[existingIndex] = { ...updatedArr[existingIndex], ...sanitizedData };
          return updatedArr;
        } else {
  ...
  }, [token, userId, safeRoom, setChatArr]);

  const handleSend = () => {
    const safeInput = sanitizeString(input.trim());
    if (!safeInput || turn !== userId) return;
  ...
  return (
    <div className="w-80 h-96 bg-neutral-900 text-white rounded-xl p-4 flex flex-col">
      {loading ? (
        ...
        <div className="flex-1 overflow-y-auto border border-neutral-700 rounded-lg p-2 mb-3 scrollbar-thin scrollbar-thumb-amber-600 scrollbar-track-neutral-800">
          {chatArr.length === 0 ? (
            <p className="text-neutral-400 text-sm">No messages yet</p>
          ) : (
            chatArr.map((chatObj) => (
      ...
      {(chatArr.length === 0 ||
        (chatArr[chatArr.length - 1]?.userId !== userId && turn === userId)) && (
        <div className="flex gap-2">
          <input
            type="text"
            value={input}
            ...
            onKeyDown={(e) => e.key === "Enter" && handleSend()}
            placeholder="Type a message..."
            className="flex-1 bg-neutral-800 border border-neutral-700 rounded-lg p-2 text-sm outline-none focus:ring-2 focus:ring-amber-500"
          />
          <button
            onClick={handleSend}
            className="bg-amber-600 hover:bg-amber-700 rounded-lg px-4 text-sm font-semibold transition-colors duration-200"
          >
            Send
          </button>
        </div>
